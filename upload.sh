...
#获取版本号并显示
VersionString=`grep -E 's.version.*=' XPUIKit.podspec`
...
LineNumber=`grep -nE 's.version.*=' XPUIKit.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" XPUIKit.podspec
echo -e "\n${Default}================================================"
...
pod repo push XPSpecs XPUIKit.podspec --verbose --allow-warnings --use-libraries
#pod repo push xyj-xyjpodspec【私库名】 XYJBaseUI.podspec【podspec文件】 --allow-warnings[允许警告]
#pod repo update Specs && pod repo push Specs XPUIKit.podspec --verbose --allow-warnings --sources='git@git.silvrr.com:iOS/Specs.git,https://github.com/CocoaPods/Specs'
# cd ~/.cocoapods/repos/Specs && git pull origin master && cd - && pod repo push Specs XPUIKit.podspec --verbose --allow-warnings --use-libraries
